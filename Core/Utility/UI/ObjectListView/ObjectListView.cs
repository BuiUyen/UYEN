 ...
 *                  - Changed the positioning of the empty list m to use all the Clinic area. Thanks to Matze.
...
namespace Sanita.Utility.UI
{
    ...
    /// </list>
    /// </remarks>
    [Designer(typeof(Sanita.Utility.UI.Design.ObjectListViewDesigner))]
    public partial class ObjectListView : ListView, ISupportInitialize
    {
        ...
        public ObjectListView()
        {
            ...
            this.SmallImageList = SanitaUtility.GetImageList();
            this.LargeImageList = SanitaUtility.GetImageList();
        ...
        private void bwAsync_WorkerCompleted(object sender, RunWorkerCompletedEventArgs e)
        {
            try
            {
                ...
                List<OLVListItem> itemList = (List<OLVListItem>)e.Result;
        ...
        private void bwAsync_Worker(object sender, DoWorkEventArgs e)
        {
            try
            {
                List<OLVListItem> itemList = null;

                IEnumerable objectsToDisplay = this.FilteredObjects;
                //IEnumerable objectsToDisplay = (IEnumerable)e.Argument;
                ...
                if (objectsToDisplay != null)
                {
                    ...
                    foreach (object rowObject in objectsToDisplay)
                    {
                        ...
                        {
                            if (itemList.Count > MaxDisplayNumber)
                            {
                                break;
                            }
                        }
        ...
        /*++Added by KhoiVV 20100628*/
        private ExBackgroundWorker m_BackgroundWorker;
        public bool isFilter = false;
        public bool isAutoSelected;
        public bool isDisableSearch;
        public int MaxDisplayNumber = -1;                  //-1 for all
        public bool isUseThread = true;
        public bool isMustUseThread = false;
        public bool isDrawSearchTextHighligh = true;
        public bool isUseNoneVietNamese = false;
        public bool isSearchWholeText = true;               //Tìm kiếm từ trong mọi vị trí (false: tìm từ đầu tiên)
        public static bool isNotUseThreadAll = false;

        private bool _isUeDevExpressStyle = false;
        public bool isUeDevExpressStyle
        {
            set
            {
                _isUeDevExpressStyle = value;
                if (value)
                {
                    //BackColor = Color.FromArgb(240, 240, 240);

                    HeaderUsesThemes = false;
                    HeaderFormatStyle = new Utility.UI.HeaderFormatStyle();
                    HeaderFormatStyle.Normal.BackColor = Color.FromArgb(0, 0, 0);
                    HeaderFormatStyle.Normal.ForeColor = Color.FromArgb(200, 200, 200);
                    //HeaderFormatStyle.Normal.FrameColor = Color.FromArgb(200, 200, 200);
                    HeaderFormatStyle.Normal.FrameWidth = 2f;

                    HeaderFormatStyle.Pressed.BackColor = Color.FromArgb(240, 240, 240);
                    HeaderFormatStyle.Pressed.ForeColor = Color.FromArgb(0, 0, 0);
                    //HeaderFormatStyle.Pressed.FrameColor = Color.FromArgb(200, 200, 200);
                    //HeaderFormatStyle.Pressed.FrameWidth = 0.2f;

                    HeaderFormatStyle.Hot.BackColor = Color.FromArgb(240, 240, 240);
                    HeaderFormatStyle.Hot.ForeColor = Color.FromArgb(0, 0, 0);
                    //HeaderFormatStyle.Hot.FrameColor = Color.FromArgb(200, 200, 200);
                    //HeaderFormatStyle.Hot.FrameWidth = 0.2f;
                }
            }
        }
        /*--Added by KhoiVV 20100628*/
        ...
        /// </summary>
        [Editor("Sanita.Utility.UI.Design.OLVColumnCollectionEditor", "System.Drawing.Design.UITypeEditor")]
        ...
        public virtual void AddObject(bool IsToFirst, object modelObject)
        {
            if (this.InvokeRequired)
                this.Invoke((MethodInvoker)delegate() { this.AddObject(IsToFirst, modelObject); });
            else
                this.AddObjects(IsToFirst, new object[] { modelObject });
        }

        public virtual void AddObjects(bool IsToFirst, ICollection modelObjects)
        {
            if (this.InvokeRequired)
            {
                this.Invoke((MethodInvoker)delegate() { this.AddObjects(IsToFirst, modelObjects); });
                return;
            }
            if (IsToFirst)
            {
                this.InsertObjects(0, modelObjects);
            }
            else
            {
                this.InsertObjects(this.GetItemCount(), modelObjects);
...
#pragma warning restore 168
        ...
        protected virtual IList<OLVGroup> MakeGroups(GroupingParameters parms)
        {
            ...
            foreach (OLVListItem olvi in parms.ListView.Items)
            {
                ...
                //++Added by KhoiVV 20101129
                //Fix viec group theo nam, thang, ngay (bo qua gio, phut, giay)
                if (key is DateTime)
                {
                    key = new DateTime(((DateTime)key).Year, ((DateTime)key).Month, ((DateTime)key).Day);
                }
                //--Added by KhoiVV 20101129
        ...
        {
            //++Add try-catch by KhoiVV 20120319
            try
            {
                //++Added by KhoiVV (2012-04-25)
                //False thì sẽ load ngay lập tức, chậm  >>CIS
                //True thì đưa vào thread, load sau tuy nhiên bên ngoài dùng Item[i].XX thì chết, vì chưa load ngay vào list >>HIS
                if ((isUseThread && !isNotUseThreadAll) || isMustUseThread)
                {
                    bwAsync_Start();
                }
                //--Added by KhoiVV //++Added by KhoiVV (2012-04-25)
                else
                {
                    if (this.Frozen)
                        return;
        ...
        {
            /*++Added by KhoiVV 100909 - Fix loi search on control when typing*/
            {
                if (isDisableSearch)
                {
                    char chr = (char)m.WParam.ToInt32(); //TODO: Will this work on 64 bit or MBCS?
                    OnPutCharData(chr);
                    return true;
                }
            }
        ...
        protected virtual bool HandleWindowPosChanging(ref Message m)
        {
            ...
            if ((pos.flags & SWP_NOSIZE) == 0)
            {
                if (pos.cx < this.Bounds.Width) // only when shrinking
                    // pos.cx is the window width, not the Clinic area width, so we have to subtract the border widths
        ...
        /// </summary>
        static public Bitmap SortAscendingImage = MediboxAssistant.Properties.Resources.SortAscending;

        /// <summary>
        /// Gets or sets the image that will be placed next to the Sort Descending command
        /// </summary>
        static public Bitmap SortDescendingImage = MediboxAssistant.Properties.Resources.SortDescending;
...
#pragma warning restore 168
        ...
        protected override bool ProcessDialogKey(Keys keyData)
        {
            ...
            if ((keyData == Keys.Return || keyData == Keys.Enter) && this.IsCellEditing)
            {
                ...
                if (keyData == Keys.Enter)
                {
                    ...
                    {
                        this.Items[this.SelectedIndex + 1].Selected = true;
                        this.FocusedItem = this.SelectedItem;

                        if (this.TopItemIndex + this.RowsPerPage < this.SelectedIndex + 1)
                        {
                            this.TopItemIndex = this.SelectedIndex + 1;
                        }
                    }
                }
                return true;
        ...
        /// <param name="item">The row to be edited</param>
        /// <param name="subItemIndex">The index of the cell to be edited</param>
        /// KhoiVV change return from void to bool
        public virtual bool StartCellEdit(OLVListItem item, int subItemIndex)
        {
            ...
            //++Added by KhoiVV 20101223
            this.SetComboControlValue(c, column.GetValue(item.RowObject), column.GetStringValue(item.RowObject));
            //--Added by KhoiVV 20101223
        ...
        //++Add by KhoiVV 20101223
        protected virtual void SetComboControlValue(Control control, Object value, String stringValue)
        {
            ...
            else if (control is DateTimePicker)
            {
                try
                {
                    ...
                    DateTime.TryParse(stringValue, out dt_value);
                    dt.Value = dt_value;
                }
                catch
                {
        ...
        {
            //++Update by KhoiVV 20101223
            if (control is ComboBoxEx)
            {
                (control as ComboBoxEx).Text = stringValue;
                return;
            }
            if (control is ComboBox)
            {
        ...
        protected virtual void ConfigureControl()
        {
            ...
            //++Add by KhoiVV 20100821
            if (this.cellEditor is TextBox)
            {
                //string txt = ((TextBox)this.cellEditor).Text;
                //((TextBox)this.cellEditor).Text = "";
                //((TextBox)this.cellEditor).AppendText(txt);
                (this.cellEditor as TextBox).Select(0, (this.cellEditor as TextBox).Text.Length);
            }
            if (this.cellEditor is FloatCellEditor)
            {
                (this.cellEditor as FloatCellEditor).Select(0, (this.cellEditor as FloatCellEditor).Text.Length);
            }
            //--Add by KhoiVV 20100821
        }
        ...
        protected virtual Object GetControlValue(Control control)
        {
            ...
            //++Update by KhoiVV
            if (control.Tag != null && control.Tag is AutocompleteMenu)
            {
                AutocompleteMenu mAutocompleteMenu = control.Tag as AutocompleteMenu;
                if (mAutocompleteMenu.IsOpen && mAutocompleteMenu.SelectedItemIndex >= 0)
                {
                    return mAutocompleteMenu.Items[mAutocompleteMenu.SelectedItemIndex];
                }
            }
            //--Update by KhoiVV
