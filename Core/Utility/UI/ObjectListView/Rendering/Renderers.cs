...
{
    //++Added by KhoiVV 20130804
    public interface IListSanita
    {
        bool IsRenderCheckbox { get; set; }
    }
    //--Added by KhoiVV 20130804
    ...
    public class BaseRenderer : AbstractRenderer
    {
        ...
        //++Added by KhoiVV 20130721

        public ImageList KhoiVVImageList
        {
            get
            {
                if (khoivv_imageList == null || khoivv_imageList.Images.Count == 0)
                {
                    khoivv_imageList = SanitaUtility.GetImageList();
                }

                return khoivv_imageList;
            }
        }
        private ImageList khoivv_imageList;
        ...
        public virtual Color GetBackgroundColor()
        {
            ...
            //++Added by KhoiVV
            if (this.Column.IsStatusColumn)
            {
                //ok
                //Không vẽ background cho các cột status vì background đè lên ảnh
            }
            //--Added by KhoiVV
            else
            {
        ...
        protected virtual void DrawImageAndText(Graphics g, Rectangle r)
        {
            ...
            //++Update by KhoiVV 20130804
            if (this.RowObject is IListSanita && !(this.RowObject as IListSanita).IsRenderCheckbox)
            {
                //Nothing
            }
            else
            ...
            //++Update by KhoiVV 20170224
            if (this.Column.IsStatusColumn)
            {
                //status thì draw ảnh, ko draw text
            }
            else
            {
    ...
    public class HighlightTextRenderer : BaseRenderer
    {
        ...
        {
            //++Added by KhoiVV
            if (this.ListView.isDrawSearchTextHighligh == false)
            {
                //Không vẽ các text được Highligh
                return;
            }
            //--Added by KhoiVV
            ...
            //++Added by KhoiVV
            if (this.ListView.isUseNoneVietNamese)
            {
                foreach (CharacterRange range in this.Filter.FindAllMatchedRanges(SanitaUtility.RemoveSign4VN(txt)))
                {
    ...
    public class BarRenderer : BaseRenderer
    {
        ...
        private Pen pen_height;
        public Pen Pen_Height
        {
            get
            {
                if (this.pen_height == null)
                    return new Pen(Color.Red, 2.0f);
                else
                    return this.pen_height;
            }
            set
            {
                this.pen_height = value;
            }
        }
        ...
        public Pen Pen_Low
        {
            get
            {
                if (this.pen_low == null)
                    return new Pen(Color.Blue, 2.0f);
                else
                    return this.pen_low;
            }
            set
        ...
        private Pen pen_normal;
        public Pen Pen_Normal
        {
            get
            {
                if (this.pen_normal == null)
                    return new Pen(Color.Black, 2.0f);
                else
                    return this.pen_normal;
            }
            set
            {
                this.pen_normal = value;
            }
        }
        ...
        public override void Render(Graphics g, Rectangle r)
        {
            ...
            //++Added by KhoiVV
            String aspectValue_XN = convertable.ToString();
            if (aspectValue_XN != null && aspectValue_XN.Split('|').Length == 3)
            {
                IList<String> list_string = aspectValue_XN.Split('|').ToList();
                double data_value = list_string[0].GetDouble();
                double data_min = list_string[1].GetDouble();
                double data_max = list_string[2].GetDouble();

                //Draw khung bao
                g.DrawRectangle(this.Pen, frameRect);

                //Draw min và draw max
                if (data_min == data_max)
                {
                    //Draw 1 vạch ở giữa
                    g.DrawLine(this.Pen, new Point(frameRect.X + (int)(frameRect.Width * 0.5), frameRect.Y), new Point(frameRect.X + (int)(frameRect.Width * 0.5), frameRect.Y + frameRect.Height));

                    if (data_value > data_max)
                    {
                        g.DrawLine(this.Pen_Height, new Point(frameRect.X + (int)(frameRect.Width * 0.75), frameRect.Y), new Point(frameRect.X + (int)(frameRect.Width * 0.75), frameRect.Y + frameRect.Height));
                    }
                    else if (data_value < data_max)
                    {
                        g.DrawLine(this.Pen_Low, new Point(frameRect.X + (int)(frameRect.Width * 0.25), frameRect.Y), new Point(frameRect.X + (int)(frameRect.Width * 0.25), frameRect.Y + frameRect.Height));
                    }
                    else
                    {
                        g.DrawLine(this.Pen_Normal, new Point(frameRect.X + (int)(frameRect.Width * 0.5), frameRect.Y), new Point(frameRect.X + (int)(frameRect.Width * 0.5), frameRect.Y + frameRect.Height));
                    }
                }
                else if (data_min < data_max)
                {
                    //Draw 2 vạch ở 1/3 và 2/3
                    g.DrawLine(this.Pen, new Point(frameRect.X + (int)(frameRect.Width * 0.3333), frameRect.Y), new Point(frameRect.X + (int)(frameRect.Width * 0.3333), frameRect.Y + frameRect.Height));
                    g.DrawLine(this.Pen, new Point(frameRect.X + (int)(frameRect.Width * 0.6666), frameRect.Y), new Point(frameRect.X + (int)(frameRect.Width * 0.6666), frameRect.Y + frameRect.Height));

                    //Init
                    double Pos_Min = frameRect.X + (int)(frameRect.Width * 0.3333);
                    double Width_Min_Max = (int)(frameRect.Width * 0.3333);
                    double Value_Min_Max = data_max - data_min;
                    double Tyle_Min_Max = Width_Min_Max / Value_Min_Max;

                    //Data
                    double Value_Value_Min = data_value - data_min;
                    double Width_Value_Min = Value_Value_Min * Tyle_Min_Max;
                    double Pos_Value = Pos_Min + Width_Value_Min;
                    if (Pos_Value < frameRect.X)
                    {
                        Pos_Value = frameRect.X + 1;
                    }
                    if (Pos_Value > frameRect.X + frameRect.Width)
                    {
                        Pos_Value = frameRect.X + frameRect.Width - 1;
                    }

                    if (data_value > data_max)
                    {
                        g.DrawLine(this.Pen_Height, new Point((int)Pos_Value, frameRect.Y), new Point((int)Pos_Value, frameRect.Y + frameRect.Height));
                    }
                    else if (data_value < data_min)
                    {
                        g.DrawLine(this.Pen_Low, new Point((int)Pos_Value, frameRect.Y), new Point((int)Pos_Value, frameRect.Y + frameRect.Height));
                    }
                    else
                    {
                        g.DrawLine(this.Pen_Normal, new Point((int)Pos_Value, frameRect.Y), new Point((int)Pos_Value, frameRect.Y + frameRect.Height));
                    }
                }
            }
            else if (String.IsNullOrEmpty(aspectValue_XN))
            {
                return;
            }
            //--Added by KhoiVV
            ...
            if (!String.IsNullOrEmpty(description))
            {
                using (StringFormat fmt2 = new StringFormat())
                {
                    ...
                    if (description.StartsWith("[X]"))
                    {
                        ...
                        {
                            g.DrawString(description.Replace("[X]", ""), this.DescriptionFontOrDefault, b, textBounds, fmt2);
                        }
                    }
                    else
                    {
